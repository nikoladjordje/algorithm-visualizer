...
export function TreeVisualizer({ state, operation }: { state?: TreeState; operation?: TreeOperation['kind'] }) {
  ...
  </svg><p className="tree-description">{description}</p><p>{state?.lookupTarget === null || state?.lookupTarget === undefined ? `${operation === 'INORDER' ? 'Inorder' : operation === 'POSTORDER' ? 'Postorder' : 'Preorder'}: ${state?.traversalOrder.join(' → ') || 'not started'}` : `Lookup path: ${state.lookupPath?.join(' → ') || 'not started'}`}</p></div>
}
