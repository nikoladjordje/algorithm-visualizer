...
describe('App algorithm workbench',()=>{
 ...
 expect(screen.getByLabelText('Target')).toHaveValue('5')
  ...
  it('runs postorder playback and explains that children are visited before their parent', async () => {
    const postorderTrace = { apiVersion: '2.0', algorithm: { id: 'binary-search-tree', name: 'Binary Search Tree', family: 'TREE' }, input: { kind: 'TREE', insertionValues: [8, 3, 10], operation: { kind: 'POSTORDER' } }, result: { kind: 'POSTORDER', visitedValues: [3, 10, 8], visitedNodeCount: 3, constructionComparisonCount: 2, constructionAttachmentCount: 3 }, limits: { maximumEvents: 10000 }, events: [{ sequence: 1, type: 'TRAVERSAL_NODE_VISITED', pseudocodeLineId: 'tree-postorder-visit', state: { kind: 'TREE', nodes: [{ id: 1, value: 8, parentId: null, leftId: 2, rightId: 3 }, { id: 2, value: 3, parentId: 1, leftId: null, rightId: null }, { id: 3, value: 10, parentId: 1, leftId: null, rightId: null }], rootId: 1, activeNodeId: 2, traversalOrder: [3], comparisonDirection: null, attachedNodeId: null }, data: { kind: 'TRAVERSAL_NODE_VISITED', nodeId: 2, parentId: null, position: null, direction: null, attachedNodeId: null } }, { sequence: 2, type: 'OPERATION_COMPLETED', pseudocodeLineId: 'tree-operation-complete', state: { kind: 'TREE', nodes: [{ id: 1, value: 8, parentId: null, leftId: 2, rightId: 3 }, { id: 2, value: 3, parentId: 1, leftId: null, rightId: null }, { id: 3, value: 10, parentId: 1, leftId: null, rightId: null }], rootId: 1, activeNodeId: null, traversalOrder: [3, 10, 8], comparisonDirection: null, attachedNodeId: null }, data: { kind: 'OPERATION_COMPLETED', nodeId: null, parentId: null, position: null, direction: null, attachedNodeId: null } }] }
    const fetchMock = vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => (void init, new Response(JSON.stringify(String(input).endsWith('/api/v2/algorithms') ? [...catalog, { ...treeCatalog, constraints: { ...treeCatalog.constraints, operations: ['PREORDER', 'INORDER', 'POSTORDER', 'LOOKUP'] } }] : postorderTrace), { headers: { 'Content-Type': 'application/json' } })))
    vi.stubGlobal('fetch', fetchMock)
    const user = userEvent.setup()
    render(<App />)
    await user.selectOptions(await screen.findByLabelText('Algorithm'), 'binary-search-tree')
    await user.selectOptions(screen.getByLabelText('Tree operation'), 'POSTORDER')
    await user.click(screen.getByRole('button', { name: 'Visualize postorder' }))
    expect(JSON.parse(String(fetchMock.mock.calls[1][1]?.body))).toEqual({ kind: 'TREE', insertionValues: [8, 3, 10, 1, 6], operation: { kind: 'POSTORDER' } })
    await user.click(screen.getByRole('button', { name: 'Next step' }))
    expect(screen.getAllByText('Visit 3 after both subtrees; postorder visits children before their parent.')).toHaveLength(2)
    await user.click(screen.getByRole('button', { name: 'Next step' }))
    expect(screen.getByText('Postorder traversal complete: 3 → 10 → 8.')).toBeInTheDocument()
    expect(screen.getByText('Postorder: 3 → 10 → 8')).toBeInTheDocument()
    expect(screen.getByText('visit left subtree, then right subtree, then node')).toBeInTheDocument()
  })
 ...
 it('cancels an in-flight trace and ignores its stale response across graph API families',async()=>{let resolveTrace!:(response:Response)=>void;const pending=new Promise<Response>(resolve=>{resolveTrace=resolve});let traceSignal:AbortSignal|undefined;const fetchMock=vi.fn((input:RequestInfo|URL,init?:RequestInit)=>{if(String(input).endsWith('/api/v2/algorithms'))return Promise.resolve(new Response(JSON.stringify(fullCatalog),{status:200,headers:{'Content-Type':'application/json'}}));traceSignal=init?.signal as AbortSignal;return pending});vi.stubGlobal('fetch',fetchMock);const user=userEvent.setup();render(<App/>);const algorithm=await screen.findByLabelText('Algorithm');await user.selectOptions(algorithm,'bfs');await user.click(screen.getByRole('button',{name:'Visualize'}));expect(screen.getByRole('button',{name:'Building…'})).toBeDisabled();await user.selectOptions(algorithm,'dijkstra');expect(traceSignal?.aborted).toBe(true);resolveTrace(new Response(JSON.stringify({apiVersion:'2.0',algorithm:{family:'GRAPH_TRAVERSAL'},events:[]}),{status:200,headers:{'Content-Type':'application/json'}}));await waitFor(()=>expect(screen.getByLabelText('Destination')).toBeInTheDocument());expect(screen.queryByRole('img',{name:/Breadth-first traversal graph/})).not.toBeInTheDocument();expect(screen.getByText('Ready')).toBeInTheDocument()})
  ...
  it('cancels an in-flight BST trace, ignores its stale response, and retains the tree draft',async()=>{let resolveTrace!:(response:Response)=>void;const pending=new Promise<Response>(resolve=>{resolveTrace=resolve});let traceSignal:AbortSignal|undefined;const catalogWithTree=[...fullCatalog,{...treeCatalog,constraints:{...treeCatalog.constraints,operations:['PREORDER','INORDER','POSTORDER','LOOKUP']}}];const fetchMock=vi.fn((input:RequestInfo|URL,init?:RequestInit)=>{if(String(input).endsWith('/api/v2/algorithms'))return Promise.resolve(new Response(JSON.stringify(catalogWithTree),{status:200,headers:{'Content-Type':'application/json'}}));traceSignal=init?.signal as AbortSignal;return pending});vi.stubGlobal('fetch',fetchMock);const user=userEvent.setup();render(<App/>);const algorithm=await screen.findByLabelText('Algorithm');await user.selectOptions(algorithm,'binary-search-tree');await user.clear(screen.getByLabelText('BST insertion sequence'));await user.type(screen.getByLabelText('BST insertion sequence'),'9, 4');await user.click(screen.getByRole('button',{name:'Visualize preorder'}));await user.selectOptions(algorithm,'bfs');expect(traceSignal?.aborted).toBe(true);resolveTrace(new Response(JSON.stringify({apiVersion:'2.0',algorithm:{family:'TREE'},events:[]}),{status:200,headers:{'Content-Type':'application/json'}}));await waitFor(()=>expect(screen.getByLabelText('Graph input')).toBeInTheDocument());expect(screen.queryByRole('img',{name:'Binary search tree'})).not.toBeInTheDocument();expect(screen.getByText('Ready')).toBeInTheDocument();await user.selectOptions(algorithm,'binary-search-tree');expect(screen.getByLabelText('BST insertion sequence')).toHaveValue('9, 4')})
